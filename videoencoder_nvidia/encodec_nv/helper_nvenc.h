...
inline void check_nvenc_error(NVENCSTATUS result, const char *const file, int const line)
{
    if (result!=NV_ENC_SUCCESS)
    {
        fprintf(stderr, "NVENC::ERROR:@%s:%d Code:=%d{%s} \n",
                file, line, result, _nvencGetErrorEnum(result) );
        exit(EXIT_FAILURE);
    }
}

inline int check_nvenc(NVENCSTATUS result, const char* desc, int const line)
{
    if (result!=NV_ENC_SUCCESS)
    {
        fprintf(stderr, "NVENC::ERROR:@:%d:<%s> {%s} \n",
                line,desc, _nvencGetErrorEnum(result) );
//        exit(EXIT_FAILURE);
        return -1;
    }
    return 0;
...
#define CHK_NVENC(val,desc)           if(check_nvenc( (val), desc, __LINE__ ))return -1;

#define CHK_NVENC_ERR(val)           check_nvenc_error ( (val), __FILE__, __LINE__ )
