...
int check_cudadrv(CUresult result,const char*desc,int line)
{
    if (result!=CUDA_SUCCESS)
    {
        fprintf(stderr, "CUDA::ERROR:@:%d:<%s> {%s} \n",
                line, desc,getCudaDrvErrorString(result) );
//        exit(EXIT_FAILURE);
        return -1;
    }
    return 0;
...
void check_cudadrv_error(CUresult result, const char *const file, int const line)
{
    if (result!=CUDA_SUCCESS)
    {
        fprintf(stderr, "CUDA::ERROR:@%s:%d Code:=%d{%s} \n",
                file, line, result, getCudaDrvErrorString(result) );
        exit(EXIT_FAILURE);
    }
}

#define CHK_CUDA_ERR(val)           check_cudadrv_error ( (val), __FILE__, __LINE__ )


#define CHK_CUDA(val,desc)           if(check_cudadrv ( (val), desc, __LINE__ ))return -1;
