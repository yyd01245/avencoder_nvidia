...
int packet_bitrate = 4*1024*1024;
int smooth_bitrate = 5*1024*1024;
...
int main(int argc,char **argv)
{
    ...
    if(SIG_ERR==signal(SIGINT,sighndl)){
        fprintf(stderr,"Install Signal ERROR\n");

    }
	
	const char *short_options = "c:i:w:h:e:b:d:p:s:g:f:l:?";
	struct option long_options[] = {
		{"help",	no_argument,	   NULL, '?'},
		{"index",	required_argument, NULL, 'i'},
		{"width",	required_argument, NULL, 'w'},
		{"height",	required_argument, NULL, 'h'},
		{"ebitrate",required_argument, NULL, 'e'},
	...
	while(1)
	{
		...
		switch(c)
		{
			case '?':
				...
				printf("-b --bitrate 		Set the packet bitrate .\n");
				printf("-e --ebitrate 		Set the encoder bitrate.\n");
                ...
                printf("-c --core       Set GPU core for performance.\n");
	...
	if(has_pbitrate == true && has_sbitrate == false)
		smooth_bitrate = packet_bitrate*11/10;
	else if(has_pbitrate == false && has_sbitrate == true)
		packet_bitrate = smooth_bitrate*9/10;
	
	//TODO 检查各个参数的有效性
	printf("avencoder version %s\n",version_str);
	printf("++++++index=%d|core=%d|width=%d|height=%d|smooth_bitrate=%d|packet_bitrate=%d|encode_bitrate=%d|destip=%s|destport=%d++++++\n",m_index,devID,width,height,smooth_bitrate,packet_bitrate,encode_bitrate,destip,destport);
    ...
    video_encoder_params.devid=devID;
    video_encoder_params.shm_addr=get_video_shm(p_video_source);
	...
	//avmuxer_params.codecID = KY_CODEC_ID_MPEG2VIDEO;
	avmuxer_params.nWidth = width;
	avmuxer_params.nHeight = height;
	avmuxer_params.nBitRate = packet_bitrate;
...
//        fprintf(stderr,"Inc:{X:%d Y:%d || W:%d W:%d}\n",x,y,w,h);
...
//		first_frame = 0;
	...
	*/
	...
	ret = encode_video_sample(p_video_encoder,p_video_source_sample,n_video_source_sample,p_video_encode_sample,&n_video_encode_sample,x,y,w,h);
	if(ret < 0)
	{		 
...
void* statistics_process(void *param)
{
	...
	while(1)
	{
		...
		memset(tmp_buffer,0,sizeof(tmp_buffer));
		snprintf(tmp_buffer,sizeof(tmp_buffer),"packet_bitrate=%d\n",packet_bitrate);
		fwrite(tmp_buffer,strlen(tmp_buffer),1,fp);
