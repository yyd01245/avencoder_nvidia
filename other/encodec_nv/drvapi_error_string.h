...
int check_cuda(CUresult result,const char*desc,int line)
{
...
void check_cuda_error(CUresult result, const char *const file, int const line)
{
    if (result!=CUDA_SUCCESS)
    {
...
#define CHK_CUDA_ERR(val)           check_cuda_error ( (val), __FILE__, __LINE__ )


#define CHK_CUDA(val,desc)           if(check_cuda ( (val), desc, __LINE__ ))return -1;
