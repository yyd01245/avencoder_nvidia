...
int pcr_adjusting(unsigned char *buffer,int buffer_len)
{
	...
	{
		int pid = AV_RB16(buffer + 1) & 0x1fff;
		if(pid == 256)
		{
			cc = (cc + 1) & 0xf;
			buffer[3] = (buffer[3]&0xf0);
...
int pcr_create(unsigned char buffer[188])//ns
{
	int pid = 256;
	int ret_int = 0;
	struct timespec  tv;
    ...
    *q++ = 188 - 5; /* Adaptation Field Length */
...
void *ts_send_thread(void *arg)
{
	...
	while(1)
	{
		...
		if(send_buffer_len <= 0)
			have_delay = delay_ms*1000;//us
		else
			have_delay = ((double)send_buffer_len/param->bit_rate)*8*1000*1000;

		int packet_number = send_buffer_len/188;
		for(int i = 0;i<packet_number;i++)
		{
			//pcr_adjusting
			ret_int = pcr_adjusting((unsigned char *)(send_buffer+i*188),188);
			//printf("debug :pcr_adjusting ret_int=%d\n",ret_int);
			
			real_send_len = sendto(socket_id, send_buffer+i*188, 188, 0,(struct sockaddr *)&servaddr, sizeof(servaddr));			
			if(real_send_len > 0)
			{
			...
			{
				printf("Waring :sentto error have_send_len=%d real_send_len=%d errno=%d !\n",188,real_send_len,errno);
			}
		}

		//pcr_create insert
		ret_int = pcr_create(pcr_packet);
		real_send_len = sendto(socket_id, pcr_packet, 188, 0,(struct sockaddr *)&servaddr, sizeof(servaddr));			
		if(real_send_len > 0)
		{
			param->real_send_bytes += real_send_len;
		}
		...
		{
			printf("Waring :sentto pcr error have_send_len=%d real_send_len=%d errno=%d !\n",188,real_send_len,errno);
		}
